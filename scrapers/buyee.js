const { fetchHTML, normaliseCondition } = require('./utils');
...
module.exports = async function scrapeBuyee({ query, maxPrice, page = 1 }) {
  ...
  $('.itemCard, .search-result-item, [class*="itemCard"]').each((_, el) => {
    try {
      ...
      const price = allNumbers.length > 0 ? parseInt(allNumbers[0].replace(/,/g, '')) : 0;
      const href = $el.find('a').first().attr('href');
      const link = href ? (href.startsWith('http') ? href : BASE + href) : null;
      const itemIdMatch = link ? link.match(/auction\/([a-z0-9]+)/) : null;
      const image = itemIdMatch ? `https://auctions.c.yimg.jp/images.auctions.yahoo.co.jp/image/dr000/auc0504/users/${itemIdMatch[1]}/i-img600x600.jpg` : null;
      ...
      if (title && price > 0 && link) {
        listings.push({ id: `buyee_${Buffer.from(link).toString('base64').slice(0, 12)}`, platform: 'buyee', platformName: 'Buyee', title, price, currency: 'JPY', image, link, condition });
      }
    } catch (e) {}
  });
